...
import {catchError, debounceTime, distinctUntilChanged, map, tap, switchMap, withLatestFrom} from 'rxjs/operators';
...
export class SearchComponent implements OnInit {
  ...
  mutlisearch(name:string){
   const mutliresults=[];
  
   var url='https://api.themoviedb.org/3/search/multi?api_key=97588ddc4a26e3091152aa0c9a40de22&language=en-US&query=love';
    this.http.get(url).subscribe((res:any)=>{
       var count=0;
       var i=0;
       console.log(res);
       while(count!=7){
         if(res['results'][i]['media_type']==="movie"||res['results'][i]['media_type']==="tv"){
            mutliresults[i]=res['results'][i];
            console.log(mutliresults[i]);
          i++;
          count++;
         }else{
           i++;
  ...
  search = (text$: Observable<string>) =>
    text$.pipe(
      debounceTime(2000),
      map(term => term === '' ? []:this.mutlisearch(term))
    )
